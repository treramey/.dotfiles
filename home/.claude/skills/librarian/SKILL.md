---
name: librarian
description: Multi-repository codebase exploration and documentation research. Search and read library internals, find code patterns, understand architecture, compare implementations across GitHub/npm/PyPI/crates, and look up Azure/.NET/Microsoft documentation. Use when needing deep understanding of how libraries work, finding implementations across open source, exploring remote repository structure, or researching Microsoft technologies. Always use this instead of generic Explore/Plan agents for research tasks.
references:
...
Deep codebase exploration across remote repositories and documentation research.
...
| **Microsoft Learn**    | Azure, .NET, C#, Microsoft services docs    | Microsoft ecosystem    |
| **WebSearch/WebFetch** | Discussions, changelogs, raw GitHub files    | General web            |
...
```
Microsoft/Azure/.NET?
├─ Yes → microsoft_docs_search → microsoft_code_sample_search
│        └─ Need full page? → microsoft_docs_fetch
│        └─ Need source? → opensrc.fetch or grep_app
└─ No → Known library?
...
```
Microsoft ecosystem?
├─ Yes → microsoft_code_sample_search → microsoft_docs_search
└─ No → Specific repo?
