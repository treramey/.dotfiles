...
- ColdBox - `~/.local-dev-docs/resources/coldbox` - MVC framework, routing, handlers, modules
- CommandBox - `~/.local-dev-docs/resources/commnadbox` - CLI, package management, embedded server, task runners
- LogBox - `~/.local-dev-docs/resources/logbox` - Logging framework, appenders, configuration
- TestBox - `~/.local-dev-docs/resources/testbox` - BDD/TDD testing framework, MockBox, test runners
...
Special instructions for ColdBox/CommandBox/LogBox/TestBox:

- These are CFML/ColdFusion frameworks from Ortus Solutions
- Check the README.md and SUMMARY.md files first to understand documentation structure
- Search the docs for answers before diving into specific files
- Use CFML syntax in code examples
